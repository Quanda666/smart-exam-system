...
SELECT e.id, c.id FROM exam e, edu_class c WHERE e.exam_name = 'Java程序设计期中测试' AND c.class_name = '23本科计科1班';

INSERT IGNORE INTO exam_attempt (exam_id, user_id, status)
SELECT e.id, u.id, 0
FROM exam e
JOIN exam_class ec ON e.id = ec.exam_id
JOIN student_profile sp ON ec.class_id = sp.class_id
JOIN sys_user u ON sp.user_id = u.id
WHERE e.exam_name = 'Java程序设计期中测试';
