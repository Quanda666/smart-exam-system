...
| 模块 | 覆盖点 |
|---|---|
| 后端编译 | Controller、Service、DTO、数据库配置、AI 上传接口、AI 日志接口、资料库/RAG 接口 |
| 前端类型检查 | Vue 组件、API 类型、AI 出题台、学生错题讲解、系统日志页、考试离线草稿 |
| 前端构建 | Vite 生产构建 |
...
| AI 题目来源追踪 | 后端编译与前端类型检查通过；AI 草稿保存后保留来源类型、来源页段、来源片段、AI 模型和提示词版本 |
| 资料库/RAG 底座 | 后端编译通过；新增资料上传、分段、大纲和基于资料生成题目接口 |
| 考试草稿保存状态 | 前端类型检查与生产构建通过；答题页新增保存状态、手动保存、失败重试、本机离线缓存和恢复提示 |
...
6. 上传课件到资料库，确认生成知识点大纲；调用资料库生成题目，检查题目来源页码/段落。
7. 创建试卷、发布考试，学生进入考试中心作答并交卷。
8. 教师阅卷主观题，调用 AI 评分建议后提交分数。
9. 学生查看成绩和错题本，调用错题 AI 讲解。
10. 管理员查看系统日志、AI 调用日志和异常事件。
