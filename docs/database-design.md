...
阶段 6 围绕考试任务和学生答题新增考试任务、考试班级、考试记录和答案记录表：

| 表名 | 说明 | 阶段 6 用途 |
|---|---|---|
| exam | 考试任务表 | 存储考试名称、关联试卷、起止时间、时长、状态和创建人 |
| exam_class | 考试班级关联表 | 存储考试任务与班级的多对多关系 |
| exam_attempt | 学生考试记录表 | 记录学生参与的某场考试，包含开始时间、提交时间、得分和状态 |
| answer_record | 学生答案记录表 | 记录学生在某场考试中对每道题的作答、得分和批阅状态 |

阶段 6 关键约束：

1. `exam` 关联已发布的 `paper`，草稿试卷不可发布考试。
2. 创建 `exam` 时，系统自动为目标班级下的所有学生创建 `exam_attempt` 记录，状态为“待开始”。
3. 学生开始考试时，`exam_attempt` 状态变为“进行中”，并记录开始时间。
4. 学生提交试卷时，`exam_attempt` 状态更新为“已提交”，客观题自动评分，主观题进入“待批阅”状态。

## 阶段 7 - 9 新增与调整

阶段 7、8、9 分别围绕阅卷、学生反馈和系统监控完善了功能，新增了批阅记录、错题本、异常事件和操作日志等表。

| 表名 | 说明 |
|---|---|
| review_record | 存储主观题的批阅分数和评语 |
| wrong_question_book | 记录每个学生的错题，用于生成错题本 |
| cheat_event | 记录学生考试中的切屏、失焦等异常行为 |
| operation_log | 记录管理员和教师的关键操作日志 |

## 后续阶段扩展

阶段 10 新增 `ai_review_suggestion` 表，用于存储 AI 对主观题的评分建议，该建议仅作为教师参考，不直接决定成绩。

| 表名 | 说明 |
|---|---|
| ai_review_suggestion | 存储 AI 对主观题的评分建议 |
