...
教师先下载模板，按“题型 / 题干 / 选项 / 答案 / 解析 / 分值”列填写，上传后后端解析并返回结构化题目草稿。题型支持“单选、多选、判断、填空、主观”中文别名；客观题选项需 `A. 内容` 每行一个，答案 `A` 或 `A,B,C`；填空与主观题提供参考答案。这是当前题库页的批量导入方式，解析在服务端完成、不依赖大模型，稳定且可校验。

> 早期的 `/api/ai/questions/import-document` 文档识别接口（上传 Word/PPT/PDF 由 AI 抽题）已移除，相关本地解析逻辑同步删除；如需从文档抽题，请先将内容整理进 Excel 模板再导入。
