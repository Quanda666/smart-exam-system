...
- `GET /api/question-template/download`：下载标准 Excel 题目模板（`.xlsx`）。
- `POST /api/question-template/import`：按模板格式批量导入题目。

教师先下载模板，按“题型 / 题干 / 选项 / 答案 / 解析 / 分值”列填写，上传后后端解析并返回结构化题目草稿。题型支持“单选、多选、判断、填空、主观”中文别名；客观题选项需 `A. 内容` 每行一个，答案 `A` 或 `A,B,C`；填空与主观题提供参考答案。这是当前题库页推荐的批量导入方式，解析在服务端完成、不依赖大模型，稳定且可校验。

### 上传题目文档识别（AI，遗留）

接口：`POST /api/ai/questions/import-document`

支持上传 `txt`、`md`、`docx`、`doc`、`pptx`、`ppt`、`pdf` 等文档，后端先抽取文本，再由 AI 识别题干、选项、答案和解析。该接口仍保留供需要从非结构化文档抽题的场景使用；题库页默认入口已切换为更稳定的 Excel 模板导入。
...
- `POST /api/materials/{id}/questions/generate`

教师上传课件或课程资料后，系统会抽取文本、切分资料段落，并生成知识点大纲。基于资料库生成题目时，系统会从资料分段中按知识点名称和补充要求做轻量检索，把带有“页/段”标记的资料上下文交给 AI 生成题目。生成结果会携带 `materialId`、`sourcePage`、`sourceParagraph`、`sourceExcerpt`、`aiModel`、`promptVersion`，便于教师审查题目依据。

### 保存 AI 草稿

接口：`POST /api/ai/questions/save`

AI 结果不会自动发布。保存后进入题库草稿状态，由教师继续编辑、确认和发布。入库题目会保留 `source_type`、`source_detail`、来源页码/段落、来源片段、AI 模型和提示词版本，区分普通手动题、AI 直接生成、题目文档识别、课程材料生成和资料库/RAG 生成。
...
- `pdf`：优先使用 PDFBox 抽取文本；若结果疑似字体、语言标记或扫描图片噪声，则直接提示无法可靠识别，避免生成假题。
