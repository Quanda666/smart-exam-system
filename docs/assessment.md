...
> **历史快照（2026-06-09）**
> 本文档是重建开发（`docs/rebuild/`）启动前的一次点时审查记录，仅供回溯。
> 项目当前进度、能力清单与开发批次的权威来源是
> [docs/rebuild/README.md](rebuild/README.md)（已记录至数十个迭代批次）。
> 阅读下方结论时请注意下列项已被后续工作覆盖：
> - “前端尚未使用 Vue Router” —— 已在 rebuild 第二批引入 `vue-router` 接管路由。
> - “AI 出题台上传题目文档识别” —— 题库页导入入口已切换为 Excel 模板导入（`/api/question-template/*`）。
> - 各“保留风险 / 后续建议”多数已在 rebuild 批次中落地，请以 rebuild 文档为准。
...
| AI 来源追踪 | AI 草稿保存到题库时写入来源类型、来源页段、来源片段、AI 模型和提示词版本 |
| 资料库/RAG | 上传资料后抽取文本、切分段落、生成知识点大纲，并支持基于资料分段生成题目 |
| 考试可靠性 | 在线答题页展示草稿保存状态，保存失败自动重试，并保留本机离线草稿与恢复提示 |
...
- 资料库/RAG 支持上传课件生成知识点大纲，并按资料分段来源生成可审查题目。
- 题库列表和导出支持查看题目来源，区分手动题、AI 生成、文档识别、材料生成和资料库/RAG。
- 学生错题本支持 AI 讲解。
- 教师阅卷支持 AI 评分建议。
